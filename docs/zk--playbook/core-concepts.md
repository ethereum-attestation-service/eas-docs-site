...
- **Offchain** → signed messages (e.g., EIP-712, DID, JWT) that live in a DB, a wallet, or locally on a device.
...
|                       | **Schema**               | Defines what fields the attestation includes.                  | `balance: number`, `account_id: string`, `threshold: number`        |
...
Think of it like this: you solved a Sudoku puzzle, and you convince someone you did it *without showing them the puzzle*. You prove the rules were followed, not the specific values.
